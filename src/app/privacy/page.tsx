  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-5 pb-24 sm:px-8">
      ...
      <div className="space-y-10 border-t border-ink-700 pt-8 text-sm leading-relaxed text-hobun-dim">
        {SECTIONS.map((n) => (
          <section key={n}>
            <h2 className="mb-3 text-base font-medium text-hobun">{t(`privacy${n}Title`)}</h2>
            <p>{t(`privacy${n}Body`)}</p>
          </section>
        ))}
      </div>

      <footer className="mt-16 border-t border-ink-700 pt-8">
        <Link href="/terms" className="font-mono text-[13px] text-hobun-faint underline underline-offset-4">
          {tNav("termsView")}
        </Link>
      </footer>
    </main>
