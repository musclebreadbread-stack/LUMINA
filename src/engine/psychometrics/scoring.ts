...
export interface FactorScore {
   ...
   * 이론적 최소·최대(10~50)를 선형으로 늘린 값일 뿐, 모집단 규준 백분위가 아니다.
...
function assertComplete(responses: ResponseMap): void {
  ...
  if (missing.length > 0) {
    throw new PsychometricsInputError(
...
/** 50문항 전체 응답에서 요인별 점수를 낸다. 하나라도 빠지면 어떤 문항인지 알려 준다. */
